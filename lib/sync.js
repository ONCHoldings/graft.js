...
Backbone.sync = function(method, model, options) {
    ...
    var fDfr = new $.Deferred();

    // This returns the promise in the thenFilter
    ...
    // this resolves it
    function onSync() {
        fDfr.resolve.apply(fDfr, _(arguments).toArray());
        model.off('error', onError); // clean up. we only want one or the other.
    }

    // this rejects it
    function onError() {
        fDfr.reject.apply(fDfr, _(arguments).toArray());
    ...
    return xhr.then(filterFn, filterFn);
};
