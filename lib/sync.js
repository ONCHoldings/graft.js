...
Backbone.sync = wrapSync(function(method, model, options) {
    ...
    var modelName = Graft.Data.request('name', model); // sync call.
    ...
    if (!isValidMethod) {
        ...
        Graft.Data.request('query', modelName)
            .then(dfr.resolve, dfr.reject);
    } else if (isCreateMethod && !hasModelId) {
        Graft.Data.request(method, modelName, params.data)
            .then(dfr.resolve, dfr.reject);
    } else if (isValidIdMethod) {
        Graft.Data.request(method, modelName, params.data.id, params.data)
            .then(dfr.resolve, dfr.reject);
    } else if (hasModelId) {
        Graft.Data.request(method, modelName, model.id)
            .then(dfr.resolve, dfr.reject);
    } else {
        dfr.reject(405, 'Method Not Allowed');
    }
