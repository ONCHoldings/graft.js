...
function compile(filename, content, client) {
    ...
    var wrapper = wrappers[kind] ? kind : 'default';
    var wrapFn = wrappers[wrapper];
    ...
    var ret = _.include(kinds, kind) ? wrapFn(opts) : content;
    verboseDebug('source :' + filename, content);
    return ret;
}
