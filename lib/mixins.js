...
_.f = require('f_underscore/f_underscore.js').f_;

var utils = {};

utils.callback = function(callback) {
    return {
...
utils.uncountable = [ 'advice', 'enegery', 'excretion', 'digestion',
...
utils.singularize = function(text) {
    if (utils.uncountable.indexOf(text.toLowerCase()) >= 0) { return text; }
    for (var i = utils.singularize.rules.length - 1; i >= 0; i--) {
        var rule = utils.singularize.rules[i];
        if (rule[0].test(text)) {
...
utils.pluralize = function(text) {
    if (utils.uncountable.indexOf(text.toLowerCase()) >= 0) { return text; }
    for (var i = utils.pluralize.rules.length - 1; i >= 0; i--) {
        var rule = utils.pluralize.rules[i];
        if (rule[0].test(text)) {
            return text.replace(rule[0], rule[1]);
        }
    }
