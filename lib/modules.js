...
var es6   = require('es6ify/compile');
...
require.extensions['.jade'] = function(module, filename) {
    ...
    Graft.bundles.templates[makeRelative(filename)] = filename;
};
...
//        module._compile(content.source, filename);
    } else {
        var content = es6(filename, fs.readFileSync(filename, 'utf8'));
        module._compile(wrapTransform.compile(content.source, filename), filename);
...
Graft._makeDirectoryTest = function(dir) {
    var dir = path.resolve(dir);
    var beginsWithDir = new RegExp("^"+dir+'.*');
    return function(checkDir) {
        var moreNodeModules = (/node_modules/)
            .test(checkDir.slice(dir.length));

        return beginsWithDir.test(checkDir) && !moreNodeModules;
    };
};
...
Graft.directory = function(dir) {
    ...
    if (this.directories.indexOf(dir) < 0) {
        ...
        this.directoryTests.push(Graft._makeDirectoryTest(dir));
...
function isGraftModule(filename) {
    return !!_(Graft.systems).find(function() {
