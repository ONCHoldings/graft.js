...
Graft.bundle = function(bundle, name, filepath) {
    var filename = name;
    var filepath;

    if (/^.\//.test(name)) {  // assume format of $CWD/$RELPATH
        filepath = filepath || process.cwd();
        filename = path.resolve(filepath + name)
    } else if (/^.\//.test(filepath)) { // assume format of $MODULE = $ABSPATH
        filename = filepath;
    }

    this.bundles[bundle][name] = filename;
}
...
Graft.load = function(dir, skipClient) {
    if (this.directories.indexOf(dir) < 0) {
        ...
        if (fs.existsSync(dir + '/client.js')) {
            ...
            Graft.bundle('client', dir + '/client.js');
        }
    }
    return this;
};
