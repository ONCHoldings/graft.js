...
function normalizePath(expose, source) {
    ...
    result.expose = path.relative(process.cwd(), result.source);
    result.expose = (/^\./).test(result.expose) ? result.expose : './' + result.expose;
...
Graft.add = function(component, filename) {
    ...
    if (bundle && Graft.bundles[bundle]) {
        var expose = (/^\./).test(relative) ? relative : './' + relative;
