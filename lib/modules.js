...
Graft.system = function(system, path, opts) {
    var config = {
        bundle: false,
        transform: 'wrap',
        extension: '.graft.js',
        name: _.capitalize(system),
        path: _.pluralize(system.toLowerCase()),
        instances: false
    };
    if (_(arguments).size() < 3) {
    ...
    if (!config.instances && config.path) {
        console.instances = '$' + config.path;
    }
    ...
    config.bundle && _([config.bundle]).chain()
        .flatten()
        .each(function(b) { Graft.bundles[b] = bundles[b] = {}; })
        .value();
...
Graft.add = function(component, filename) {
    ...
    Graft.bundles[kind] && (Graft.bundles[kind][component.title] = component.files);
